    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="relative flex flex-1 pb-20 md:pb-0" style={{ minHeight: "100dvh" }}>
        <div className="absolute inset-0">
          <CityScene onSelectPOI={handleSelectPOI} origemId={origemId}>
            <RouteLine
              rotaIds={rotaIds}
        ...
        {/* Badge de seleção de origem/destino */}
        ...
        {/* ── Botão COMEÇAR ─────────────────────────────────────────────── */}
        <AnimatePresence>
          {rotaIds.length > 0 && !animando && !rotaConcluida && (
            <motion.button
              key="btn-comecar"
              ...
              className="absolute bottom-10 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 rounded-2xl px-7 py-3.5 text-sm font-semibold text-white shadow-lg transition-transform active:scale-95 md:bottom-8"
          ...
          {rotaConcluida && (
            <motion.div
              ...
              className="absolute bottom-10 left-1/2 z-20 -translate-x-1/2 rounded-2xl bg-white px-6 py-3 text-sm font-semibold text-gray-800 shadow-lg md:bottom-8"
            >
              🏁 Chegou ao destino!
            </motion.div>
          )}
              ...
              {modal === "profile" && (
                <div>
                  <div className="mb-6 flex items-center gap-4">
                    ...
                    >
                      {(
                        stored?.nome?.[0] ?? (kind === "motorista" ? "M" : "P")
                      ).toUpperCase()}
                    </div>
                    <div>
